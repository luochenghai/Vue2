...
// vue 通过 initLifecycle 函数向实例中挂载属性，该函数接收Vue.js 实例作为参数。
export function initLifecycle (vm: Component) {
  const options = vm.$options

  // locate first non-abstract parent 找到第一个非抽象类型的父级
...
export function lifecycleMixin (Vue: Class<Component>) {
  Vue.prototype._update = function (vnode: VNode, hydrating?: boolean) {
    ...
    if (!prevVnode) {
      // initial render 首次初始化渲染，将vnode 变成真实dom后追加到宿主元素上
...
// mountComponent 函数的作用：将vnode 变成真实的 dom
/**
...
export function mountComponent (
  ...
  //这个 Watcher 全局就只有一个，相当于总裁秘书，服务与$root 根组件
...
// 我们使用hook从vm.$options 中获取钩子函数列表后赋值给handlers,随后遍历handlers,执行每一个钩子函数。
